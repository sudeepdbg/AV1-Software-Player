...
Deploy:
  Streamlit Community Cloud → add packages.txt containing: ffmpeg
  HF Spaces (Streamlit SDK) → same packages.txt trick
  Note: AI enhancements work best locally with GPU support
"""

import os, json, subprocess, tempfile, time, re, math
...
.enhance-header {
    ...
    margin-bottom: 10px; font-weight: 600; color: #1e293b;
}
.enhance-desc { font-size: 0.82rem; color: #64748b; margin-bottom: 12px; }

/* Toggle switch styling */
.stToggle > label { font-weight: 600; color: #1e293b; }
...
.ba-card h4 { margin: 0 0 10px; font-size: 0.9rem; color: #334155; }
.ba-metric { font-size: 1.4rem; font-weight: 700; color: #1e293b; margin: 4px 0; }
.ba-label { font-size: 0.75rem; color: #64748b; text-transform: uppercase; letter-spacing: 0.05em; }
.ba-improvement {
    color: #166534; font-weight: 600; font-size: 0.85rem; margin-top: 6px;
}
.ba-improvement.down { color: #b91c1c; }
</style>
...
def build_enhance_filters(settings: dict, src_meta: dict) -> list:
    ...
    if settings.get("denoise"):
        ...
        # hqdn3d: high quality 3D denoise (fast, CPU-friendly)
    ...
    if settings.get("sharpen"):
        ...
        # unsharp filter: amount (-1.5 to 1.5), threshold (0-255)
        filters.append(f"unsharp=5:5:{amount}:{threshold}")
    
    # ── 3. AI Upscaling / Super-Resolution ───────────────────────────────
    if settings.get("upscale"):
        ...
        algo = settings.get("upscale_algo", "lanczos")  # lanczos, spline, bicubic
        filters.append(f"scale={target_w}:{target_h}:flags={algo}+accurate_rnd+full_chroma_int")
    
    # ── 4. HDR Conversion / Tonemapping ──────────────────────────────────
    if settings.get("hdr_convert") and src_meta.get("bit_depth", 8) >= 10:
        # Convert HDR10/HLG to SDR with perceptual tonemapping
    ...
    if settings.get("color_enhance"):
        vibrance = settings.get("vibrance", 0.15)  # -0.5 to 0.5
        contrast = settings.get("contrast", 1.0)    # 0.5 to 2.0
        filters.append(f"eq=contrast={contrast}:saturation={1.0 + vibrance}")
    
    # ── 6. Artifact Reduction (Deblocking) ───────────────────────────────
    if settings.get("deblock"):
        strength = settings.get("deblock_strength", 5)  # 0-10
        filters.append(f"deblock=filter=strong:alpha={strength}:beta={strength//2}")
    
    # ── 7. Frame Interpolation (Smooth Motion) ───────────────────────────
    if settings.get("frame_interp"):
        target_fps = settings.get("target_fps", 60)
        # minterpolate: motion-compensated frame interpolation
...
def estimate_processing_time(src_meta: dict, settings: dict) -> str:
    ...
    if settings.get("frame_interp"): base_factor *= 3.0  # Very intensive
...
# ══════════════════════════════════════════════════════════════════════════════
#  ✨ AI ENHANCEMENT PANEL (Vnova-Style)
# ══════════════════════════════════════════════════════════════════════════════
if enable_encoding:
    ...
    with enh_col1:
        ...
        with st.container():
            ...
            st.markdown('<div class="enhance-header">🧹 Denoise <input type="checkbox" ' + 
                       f'{"checked" if st.session_state.enhance_settings["denoise"] else ""} + 
                       f'disabled style="margin-left:auto"></div>', unsafe_allow_html=True)
            denoise = st.checkbox("Enable Denoise", key="chk_denoise", 
            ...
            if denoise:
                st.session_state.enhance_settings["denoise_strength"] = st.slider(
                    ...
                    help="Higher = more noise reduction, but may soften details")
            st.markdown('<div class="enhance-desc">Reduces grain, compression artifacts, and sensor noise using 3D temporal filtering.</div>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
        
        # Sharpening
        with st.container():
            st.markdown('<div class="enhance-card">', unsafe_allow_html=True)
            st.markdown('<div class="enhance-header">🔍 Detail Enhancement</div>', unsafe_allow_html=True)
            sharpen = st.checkbox("Enable Sharpening", key="chk_sharpen",
            ...
            if sharpen:
                ...
                with c1:
                    st.session_state.enhance_settings["sharpen_amount"] = st.slider(
                        ...
                        help="Negative = blur, Positive = sharpen")
                with c2:
                    st.session_state.enhance_settings["sharpen_threshold"] = st.slider(
                        ...
                        help="Only sharpen edges above this contrast threshold")
            st.markdown('<div class="enhance-desc">Enhances edge definition and fine details using adaptive unsharp masking.</div>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
        
        # AI Upscaling
        with st.container():
            st.markdown('<div class="enhance-card">', unsafe_allow_html=True)
            st.markdown('<div class="enhance-header">🔬 AI Upscaling</div>', unsafe_allow_html=True)
            upscale = st.checkbox("Enable Upscaling", key="chk_upscale",
            ...
            if upscale:
                st.session_state.enhance_settings["upscale_algo"] = st.selectbox(
                    ...
                    index=["lanczos","spline","bicubic"].index(st.session_state.enhance_settings["upscale_algo"]),
                    help="Lanczos = best quality, Spline = balanced, Bicubic = fastest")
                ...
                st.session_state.enhance_settings["upscale_height"] = st.number_input(
                    ...
                    step=meta["height"])
            st.markdown('<div class="enhance-desc">High-quality resolution upscaling with edge-preserving interpolation. For true AI super-resolution, integrate Real-ESRGAN externally.</div>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
    ...
    with enh_col2:
        ...
        with st.container():
            st.markdown('<div class="enhance-card">', unsafe_allow_html=True)
            st.markdown('<div class="enhance-header">🌈 HDR → SDR Tonemapping</div>', unsafe_allow_html=True)
            hdr = st.checkbox("Enable HDR Conversion", key="chk_hdr",
            ...
            if hdr and meta.get("bit_depth", 8) >= 10:
                st.session_state.enhance_settings["tonemap_algo"] = st.selectbox(
                    ...
                    index=["hable","reinhard","mobius","linear"].index(st.session_state.enhance_settings["tonemap_algo"]),
                    help="Hable = filmic, Reinhard = balanced, Mobius = vibrant")
            elif meta.get("bit_depth", 8) < 10:
                st.caption("💡 Source is SDR (8-bit). HDR conversion requires 10-bit+ HDR10/HLG source.")
            st.markdown('<div class="enhance-desc">Converts HDR10/HLG to SDR with perceptual tonemapping for optimal display on standard screens.</div>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
        
        # Color Enhancement
        with st.container():
            st.markdown('<div class="enhance-card">', unsafe_allow_html=True)
            st.markdown('<div class="enhance-header">🎨 Color Enhancement</div>', unsafe_allow_html=True)
            color = st.checkbox("Enable Color Boost", key="chk_color",
            ...
            if color:
                ...
                with c1:
                    st.session_state.enhance_settings["vibrance"] = st.slider(
                        ...
                        help="Intelligent saturation boost (preserves skin tones)")
                with c2:
                    st.session_state.enhance_settings["contrast"] = st.slider(
                        "Contrast", 0.5, 2.0, st.session_state.enhance_settings["contrast"], 0.05)
            st.markdown('<div class="enhance-desc">Enhances color vibrancy and contrast while preserving natural skin tones and avoiding clipping.</div>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
        
        # Artifact Reduction
        with st.container():
            st.markdown('<div class="enhance-card">', unsafe_allow_html=True)
            st.markdown('<div class="enhance-header">🧩 Artifact Reduction</div>', unsafe_allow_html=True)
            deblock = st.checkbox("Enable Deblocking", key="chk_deblock",
            ...
            if deblock:
                st.session_state.enhance_settings["deblock_strength"] = st.slider(
                    ...
                    help="Higher = stronger blocking artifact removal")
            st.markdown('<div class="enhance-desc">Reduces macroblocking, ringing, and banding artifacts from heavy compression.</div>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
    
    # Frame Interpolation (advanced, resource-intensive)
    with st.expander("🎞️ Advanced: Frame Interpolation (Motion Smoothing)"):
        ...
        if frame_interp:
            ...
            st.warning("⚠️ Frame interpolation is CPU/GPU intensive and may increase processing time 3-5x. Best for short clips.")
        st.caption("Creates intermediate frames using motion compensation for smoother playback. Ideal for converting 24fps film to 60fps video.")
    ...
    if active_enhancements > 0:
        ...
        st.markdown(f"""
        <div class="proc-estimate ai">
            <b>⏱️ Estimated Processing Impact:</b> {est_time} 
            <span style="margin-left:12px;opacity:0.8">
                ({active_enhancements} enhancement{"s" if active_enhancements>1 else ""} active)
            </span>
        </div>""", unsafe_allow_html=True)
        ...
        st.markdown(f'<div style="margin:8px 0"><span class="enh-preview">{" + ".join(enh_names)}</span></div>', unsafe_allow_html=True)
    ...
    # Preview button (shows before/after metrics estimate)
    with b1:
        if st.button("🔍 Preview Impact", use_container_width=True):
            # Quick probe of what the enhanced output would look like
            est_meta = meta.copy()
            ...
            with bc1:
                st.markdown('<div class="ba-card"><div class="ba-label">Source</div><div class="ba-metric">{meta["width"]}×{meta["height"]}</div><div style="font-size:0.8rem;color:#64748b">@ {meta["fps"]} fps</div></div>', unsafe_allow_html=True)
            with bc2:
                st.markdown(f'<div class="ba-card"><div class="ba-label">Enhanced</div><div class="ba-metric">{est_meta["width"]}×{est_meta["height"]}</div><div style="font-size:0.8rem;color:#64748b">@ {est_meta["fps"]} fps</div>', unsafe_allow_html=True)
                if est_meta["width"] > meta["width"]:
                    up_pct = ((est_meta["width"]/meta["width"])**2 - 1)*100
                    st.markdown(f'<div class="ba-improvement">+{up_pct:.0f}% more pixels</div>', unsafe_allow_html=True)
                else:
                    st.markdown('<div class="ba-improvement down">Same resolution</div>', unsafe_allow_html=True)
                st.markdown('</div>', unsafe_allow_html=True)
    ...
    if go:
        ...
        bar.container().markdown('<style>.stProgress.ai > div > div { background: linear-gradient(90deg, #7c3aed, #a855f7); }</style>', unsafe_allow_html=True)
        ...
        if not ok:
            ...
            qual_improvement = {}
            if qual["vmaf"] and meta.get("vmaf_baseline"):
                qual_improvement["vmaf"] = qual["vmaf"] - meta["vmaf_baseline"]
            # Note: We'd need to pre-compute source VMAF against a reference for true delta
...
with tab_chart:
    ...
    # Smart insights with enhancement awareness
    if len(results) > 1:
        st.markdown("---")
        st.markdown("**🔍 Smart Insights**")
        
        # Best quality with enhancements noted
